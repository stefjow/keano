...
- **Non-exact nesting.** Aperture-7 subdivision means children poke past
  ...
  as spatial containers; keano never does. The parent's `m` is the mean
...
- **Pentagons.** The 12 pentagons per resolution break code that assumes 7
  children or 6 neighbors. keano computes means over *actual* children via
...
hierarchy. S2: exact geometric nesting (which keano does not need) at the
